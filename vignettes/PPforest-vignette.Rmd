...
```{r libraries, cache = FALSE, Eecho = FALSE, message = FALSE, warning = FALSE}
require(PPforest)
require(dplyr)
...
```{r,fig.show='hold', fig.height = 5, fig.width = 5, echo=FALSE}
...
To describe the data structure a parallel plot can be done, the data were standarized and the color represents the class variable.
...
mdspl2d <- function(ppf, lege = "bottom", siz = 3, k = 2) {
  ...
  mds <- ggplot2::ggplot(data = df) +
   ggplot2::geom_point(ggplot2::aes(x = MDS1, y = MDS2, color = Type),
              size = I(siz), alpha = .5) +
