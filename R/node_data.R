...
ind_node <- function(PPclassOBJ,
  ...
  if (TS[node.id, 2] != 0) {
    ...
    plot.data <-
      data.frame(
        proj.data = proj.data,
        origclass = proj.class,
        cut = cut.off[TS[node.id, 4], Rule],
        node.id = node.id,
        LR.class,
        Dir = as.factor(proj.data > cut.off[TS[node.id, 4], Rule])
      )
  ...
  nn <- data.frame(nn = 1:sum(ppf[["output.trees"]][[tr]]$Tree.Struct[, 4]!=0))
  densf <- function(x) {
  ...
  dat_pl <- apply(nn, 1, densf) %>% lapply(data.frame) %>% dplyr::bind_rows()
