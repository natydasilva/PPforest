  ...
  if(.Platform$OS.type == "windows") {
    if(class(object) == "PPforest"){
      votes <- plyr::ldply(object[[8]], function(x)
        as.numeric(PPclassify2(Tree.result = x, test.data = xnew, Rule = 1)[[2]] ))[,-1]
    }else{
      votes <- plyr::ldply(object, function(x) as.numeric(PPclassify2(Tree.result = x[[1]], test.data = xnew, Rule = 1)[[2]]))[,-1]
